...
static void TblisNotAvailable(benchmark::State &state) {
  state.SkipWithError("nda::tensor::contract benchmark requires TBLIS support");
}
...
namespace {
  ...
  template <typename Value>
  void tblis_contract(Value alpha, nda::array<Value, 3> const &a, std::string_view idx_a, nda::array<Value, 3> const &b,
                       std::string_view idx_b, Value beta, nda::matrix<Value> &c, std::string_view idx_c) {
  ...
  static void TblisTensorContract(benchmark::State &state) {
    ...
    if constexpr (std::is_same_v<real_t, float>) tolerance = std::max(tolerance, real_t{5e-3});
    else tolerance                                    = std::max(tolerance, real_t{1e-10});
...
BENCHMARK_TEMPLATE(TblisTensorContract, float)
   ->Args({64, 64, 64, 64})
   ->Args({64, 64, 64, 64})
   ->Args({64, 96, 96, 24})
