...
namespace nda::tensor::nda_tblis {
  ...
  struct tensor : ::tblis::tblis_tensor {
    ...
    explicit tensor(nda::MemoryArrayOfRank<Rank> auto &&a)
      ...
      configure_tensor(const_cast<std::decay_t<ValueType> *>(a.data()), ValueType{1});
    }

    explicit tensor(nda::MemoryArrayOfRank<Rank> auto &&a, ValueType val)
       : ::tblis::tblis_tensor(), lens_(to_lens(a.shape())), strides_(to_strides(a.strides())) {
      configure_tensor(const_cast<std::decay_t<ValueType> *>(a.data()), val);
    }
  ...
  private:
    ...
    void configure_tensor(std::decay_t<ValueType> *ptr, ValueType alpha) {
